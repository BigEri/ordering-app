...
async function allocateNewPairingCode(
  ...
  if (replaceUnused) {
    await prisma.devicePairingCode.deleteMany({ where: { deviceId, usedAtIso: null } });
  } else {
    await prisma.devicePairingCode.deleteMany({
      where: { deviceId, usedAtIso: null, expiresAtIso: { lte: nowIso() } },
    });
  }
 ...
 *
 * Bez advisory locku / interactive transaction — na Neon poolu `pg_advisory_xact_lock`
 * padá a tablet pak neukáže žádný kód.
 */
...
export async function upsertDevicePairingCodeAsync(
  ...
  if (opts?.rotate) {
    return allocateNewPairingCode(id, true);
  }

  const existing = await getUnusedPairingCodeForDeviceAsync(id);
  if (existing) {
  ...
  return allocateNewPairingCode(id, false);
}
