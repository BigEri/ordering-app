...
/**
 * Legacy /admin/welcome — middleware redirects when cookie is set.
 * Client fallback resolves restaurant from /api/admin/me.
 */
export default function AdminWelcomeRedirectPage() {
  React.useEffect(() => {
    let cancelled = false;
    void (async () => {
      try {
        ...
        window.location.replace(
          rid
            ? `/admin/restaurants/${encodeURIComponent(rid)}?tab=welcome`
            : "/admin",
        );
      } catch {
  ...
  return (
    <main className="adminPage">
      <p className="textMuted">Přesměrování na úvodní stránku provozovny…</p>
    </main>
  );
}
